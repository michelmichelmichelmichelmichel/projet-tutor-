...
export class ApiService {
    ...
    async searchCommunes(query) {
        ...
        if (this.currentCountryCode === 'fr') {
            ...
            const url = `https://geo.api.gouv.fr/communes?nom=${encodeURIComponent(query)}&fields=nom,code,codeDepartement,codeRegion,codesPostaux,population&format=geojson&geometry=contour&boost=population&limit=10`;
            try {
                const response = await fetch(url);
                ...
                return data.features.map(feature => {
                    ...
                    return {
                        ...
                        hierarchy: {
                            country: 'France',
                            region: props.codeRegion ? this.REGION_NAMES[props.codeRegion] : null,
                            dept: props.codeDepartement ? this.DEPARTEMENT_NAMES[props.codeDepartement] : null,
                            city: props.nom
                        },
                        geometry: geometry,
                        bounds: [[minLat, minLon], [maxLat, maxLon]],
